...
In `Vala.Complier.run ()`, `Vala.CCodeCompiler.compile ()` triggers the compilation progress then, the Vala compiler quits.
...
`Vala.CCodeCompiler` has one method (`compile ()`) which handles the compilation process.
...
The space-separated string is then used to obtain compile flags from pkg-config for the packages, using a `pkg-config` command. This is done by calling the `Vala.CodeContext.pkg_config_compile_flags ()` method.
...
The source files are obtained using `Vala.CodeContext.get_source_files ()` and `Vala.CodeContext.get_c_source_files ()`.
