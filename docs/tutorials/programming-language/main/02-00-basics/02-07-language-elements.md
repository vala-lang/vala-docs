...
The naming convention for methods in Vala is [snake case](https://en.wikipedia.org/wiki/Snake_case), 
or in other words, all lowercase characters with underscores as word separators. 
This may be a little bit unfamiliar to C# or Java programmers who are accustomed to 
[camelCase](https://en.wikipedia.org/wiki/Camel_case) method names like `drawShape` or 
[PascalCase](https://en.wikipedia.org/wiki/PascalCase) method names like `DrawShape`. 
...
different signature within the same scope, which is commonly referred to as 
...
void f (int x, string s) {
    f (x, s, 0.5); // not possible
}
void f (int x) {
    f (x, "hello"); // not possible
}
```
...
although it is not necessarily recommended.  You will learn how to do that later.  
...
Any method that matches this signature may be assigned to a variable of this type or passed as a method argument 
of this type.
...
namespace by following the same naming convention, e.g. `class NameSpace1.Test { ... }`. Note that when doing this, the
